...
if [ -f "$INCOMING_ENV" ]; then
  for key in CORS_ORIGIN PUBLIC_APP_URL PRIVACY_POLICY_URL ADMIN_BOOTSTRAP_PASSWORD ADMIN_PIN PREMIUM_MONTHLY_USD PREMIUM_QUARTERLY_USD PREMIUM_YEARLY_USD PREMIUM_PRICES_APPROVED STRIPE_SECRET_KEY STRIPE_PUBLIC_KEY STRIPE_WEBHOOK_SECRET STRIPE_SUCCESS_URL STRIPE_CANCEL_URL FIREBASE_PROJECT_ID FIREBASE_CLIENT_EMAIL FIREBASE_PRIVATE_KEY FIREBASE_API_KEY FIREBASE_AUTH_DOMAIN FIREBASE_STORAGE_BUCKET FIREBASE_MESSAGING_SENDER_ID FIREBASE_APP_ID FIREBASE_MEASUREMENT_ID TELEGRAM_BOT_TOKEN TELEGRAM_GAME_URL TELEGRAM_BOT_POLLING_ENABLED TELEGRAM_BOT_INSTANCE_ID TELEGRAM_DROP_PENDING_UPDATES AD_BALANCE_CAP UNLOCK_AD_GAMES UNLOCK_COLLECTION_GAMES UNLOCK_VOICE_GAMES UNLOCK_BARABAN_GAMES UNLOCK_REFERRAL_GAMES UNLOCK_PREMIUM_GAMES UNLOCK_GOLD_SHOP_WINS GDEVELOP_SILVER_ACTIVE ANDROID_TARGET_SDK GOOGLE_PLAY_BILLING_MAJOR ANDROID_RELEASE_KEYSTORE_READY GOOGLE_SERVICE_ACCOUNT_JSON APPLE_SHARED_SECRET ADMOB_ANDROID_APP_ID ADMOB_REWARDED_ANDROID_ID ADMOB_IOS_APP_ID ADMOB_REWARDED_IOS_ID ADMOB_HALAL_CATEGORIES_BLOCKED; do
...
set_env_value ANDROID_TARGET_SDK 36
set_env_value GOOGLE_PLAY_BILLING_MAJOR 8
set_env_value AD_BALANCE_CAP 50000
cp .env "$APP_ROOT/.env"
