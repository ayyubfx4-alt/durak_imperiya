...
export default function AdminLayout({ user }) {
  ...
  const linkClass = ({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${
    isActive ? 'bg-[#f5a623]/14 text-[#f5a623] ring-1 ring-[#f5a623]/30' : 'text-slate-400 hover:bg-white/[.04] hover:text-white'
  }`;

  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white lg:pl-60">
      <aside className={`fixed inset-y-0 left-0 z-40 w-60 border-r border-[#1e1e2e] bg-[#0d0d14] transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-full flex-col">
          <div className="border-b border-[#1e1e2e] p-4">
            <div className="text-xl font-black tracking-wide text-[#f5a623]">ADMIN PANEL</div>
                ...
                <div className="space-y-1">
                  {items.map(([label, to, icon]) => (
                    <NavLink key={to} to={to} className={linkClass} onClick={() => setOpen(false)}>
                      <span className="grid h-6 w-6 place-items-center rounded bg-white/[.04] text-xs font-black">{icon}</span>
                      <span>{label}</span>
                    </NavLink>
                  ))}
                </div>
      ...
      <header className="sticky top-0 z-20 border-b border-[#1e1e2e] bg-[#0a0a0f]/95 backdrop-blur">
        <div className="flex min-h-16 items-center justify-between gap-3 px-4 lg:px-6">
          <button className="btn lg:hidden" onClick={() => setOpen(true)}>Menu</button>
...
function HeaderMetric({ label, value, live }) {
  return (
    <div className="rounded border border-[#1e1e2e] bg-[#13131a] px-3 py-2">
